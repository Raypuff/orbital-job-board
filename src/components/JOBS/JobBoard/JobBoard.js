        ...
        if (jobs && jobs.length > 0) {
            ...
            if (filterState.longTerm || filterState.adHoc) {
                processedJobs = processedJobs
                    .filter(
                        (job) =>
                            filterState.longTerm || job.type !== "Long term"
                    )
                    .filter(
                        (job) => filterState.adHoc || job.type !== "Ad hoc"
                    );
            }
            if (filterState.physical || filterState.virtual) {
                processedJobs = processedJobs
                    .filter(
                        (job) =>
                            filterState.physical || job.platform !== "Physical"
                    )
                    .filter(
                        (job) =>
                            filterState.virtual || job.platform !== "Virtual"
                    );
            }
            ...
            if (filterState.sort === "mostRecent") {
                ...
                processedJobs = processedJobs.sort((job1, job2) => 0);
            }
